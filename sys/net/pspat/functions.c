...
extern void *dn_ht_find(struct dn_ht *ht, uintptr_t key, int flags,
		void *arg);
extern struct dn_sch_inst *ipdn_si_find(struct dn_schk *s,
 ...
 * to decide when to insert an entry in the CL.
 */
	...
	if (m == NULL || pspat_mb_empty(m)) {
		...
		if (m) {
			if (ENTRY_EMPTY(pq->inq->entry)) {
				/* insert into the list of mb to be clear */
...
/* move mbf to the a dispatcher queue */
static int
